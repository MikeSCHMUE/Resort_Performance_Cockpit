...
with st.form("input_form"):
    ...
    beach_high_price = st.slider("High Season Average Daily Rate (Beachfront)", 110, 180, 140, step=10)
    beach_low_price = st.slider("Low Season Average Daily Rate (Beachfront)", 90, 160, 110, step=10)
    beach_high_occ = st.slider("High Season Occupancy (Beachfront) %", 0, 100, 60, step=5) / 100
    beach_low_occ = st.slider("Low Season Occupancy (Beachfront) %", 0, 100, 40, step=5) / 100
    ...
    compact_high_price = st.slider("High Season Average Daily Rate (Compact)", 80, 150, 110, step=10)
    compact_low_price = st.slider("Low Season Average Daily Rate (Compact)", 60, 130, 80, step=10)
    compact_high_occ = st.slider("High Season Occupancy (Compact) %", 0, 100, 60, step=5) / 100
    compact_low_occ = st.slider("Low Season Occupancy (Compact) %", 0, 100, 40, step=5) / 100
    ...
    fixed_costs = st.number_input("Monthly Fixed Costs in USD (OPEX = Operational Expenditures)", value=4000.0, step=500.0)
    variable_cost = st.number_input("Variable Cost per Room per Night in USD (Toiletries, Breakfast incl.)", value=10.0, step=5.0)
    capex = st.number_input("Total Investment in USD (CAPEX = Capital Expenditure)", value=300000.0, step=10000.0)
    
    years = st.slider("Forecast Years", 1, 12, 10)
    growth = st.slider("Annual Revenue Growth (%)", 0, 10, 2) / 100
...
if st.session_state["results"]:
    ...
    price_range = np.arange(80, 141, 20)
    occ_range = np.linspace(0.4, 0.7, 4)
    heatmap = []
