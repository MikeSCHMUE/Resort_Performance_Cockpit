...
with st.form("input_form"):
    ...
    beach_high_price = st.slider("High Season ADR (Beachfront)", 120, 170, 150, step=10)
    beach_low_price = st.slider("Low Season ADR (Beachfront)", 100, 150, 120, step=10)
    ...
    compact_high_price = st.slider("High Season ADR (Compact)", 80, 150, 120, step=10)
    compact_low_price = st.slider("Low Season ADR (Compact)", 60, 130, 90, step=10)
...
if "results" in st.session_state:
    ...
    if df_profit is not None and "Profit" in df_profit.columns:
        ...
        def score_payback(val): return "🟢" if val <= 6 else "🟠" if val <= 9 else "🔴"
        ...
        metrics_data = {
            ...
            "Value": [
                f"{r['roi']:.2f}%" if r.get("roi") is not None else "N/A",
                f"{r['bep_nights']:.0f} Nights" if r.get("bep_nights") is not None else "N/A",
                f"{irr * 100:.2f}%" if irr is not None else "N/A",
                f"${npv:,.2f}" if npv is not None else "N/A",
...
if "results" in st.session_state:
    ...
    if all(k in r for k in ["nights_high", "nights_low", "total_nights", "revenue_rooms", "revenue_ancillary", "total_revenue", "total_costs", "profit", "roi", "payback", "bep_nights", "df", "capex"]):
        ...
        key_metrics = [
            ("Return on Investment (ROI)", f"{r['roi']:.2f}%"),
            ("Break-Even Point (Nights)", f"{r['bep_nights']:.0f}" if r["bep_nights"] else "N/A"),
            ("Internal Rate of Return (IRR)", f"{irr * 100:.2f}%" if irr else "N/A"),
            ("Net Present Value (NPV)", f"${npv:,.2f}" if npv else "N/A"),
